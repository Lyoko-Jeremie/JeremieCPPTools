...
{
    public:
        RWLock( bool AreLockWriter):LockWriter(AreLockWriter), ATMStopReader(false), ATMNumReader(0), ATMStopWriter(false), ATMNumWriter(0) {}  /// atomic 要赋初值
        ~RWLock() = default;
...
//                std::this_thread::sleep_for (std::chrono::microseconds(1));
        ...
        void WriteLock()
        {
//            cout << "RWLock::try lock MTXATM" << endl;
            {
                std::lock_guard<std::mutex> lg(MTXATM);
                // guard
...
//            cout << ATMNumWriter.load() << endl;
//            cout << ATMStopWriter.load() << endl;
//            cout << TurnR << endl;
//            cout << "RWLock::try while" << endl;
            while ( true  )
            {
                {
//                    cout << "RWLock::in while try lock MTXATM" << endl;
                    std::lock_guard<std::mutex> lg(MTXATM);
                    // guard
...
//                    cout << ( ATMStopWriter == true && TurnR == true ) << endl;
//                    cout << ( 0 != ATMNumReader ) << endl;
...
//                        ATMWriter = true;
                ...
                // Wait
//                cout << "RWLock::in while sleep_for" << endl;
                std::this_thread::sleep_for (std::chrono::milliseconds(1));
            }
//            ++ATMNumWriter;

            /// 这里可能会造成读者饥饿
//            cout << "RWLock::lock MTXWriter" << endl;
            if (LockWriter)
            {
                MTXWriter.lock();
            }
//            cout << "RWLock::return" << endl;
            return;
        }
